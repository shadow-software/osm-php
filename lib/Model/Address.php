...
'curbside' => 'getCurbside'    ];
    ...
    const CURBSIDE_RIGHT = 'right';
const CURBSIDE_LEFT = 'left';
const CURBSIDE_ANY = 'any';
     ...
     * @return string[]
     */
    public function getCurbsideAllowableValues()
    {
        return [
            self::CURBSIDE_RIGHT,
self::CURBSIDE_LEFT,
self::CURBSIDE_ANY,        ];
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['curbside'] = isset($data['curbside']) ? $data['curbside'] : 'any';
    }
    ...
    public function listInvalidProperties()
    {
        ...
        $allowedValues = $this->getCurbsideAllowableValues();
        if (!is_null($this->container['curbside']) && !in_array($this->container['curbside'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value for 'curbside', must be one of '%s'",
    ...
    {
        return $this->container['curbside'];
    }

    /**
     * Sets curbside
     *
     * @param string $curbside Optional parameter. Specifies on which side a point should be relative to the driver when she leaves/arrives at a start/target/via point. Only supported for motor vehicles and OpenStreetMap.
     *
     * @return $this
     */
    public function setCurbside($curbside)
    {
        $allowedValues = $this->getCurbsideAllowableValues();
        if (!is_null($curbside) && !in_array($curbside, $allowedValues, true)) {
        ...
        $this->container['curbside'] = $curbside;
